  ...
  Future<void> _verifyReceipt() async {
    ...
    if (_tokenIdController.text.isEmpty || serial == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content:
                Text('Please fill all fields and ensure serial is a number')),
      );
      return;
    }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          _buildTip('1', 'Enter Token ID',
              'Paste the unique identifier of your NFT receipt.'),
          const SizedBox(height: 12),
          _buildTip('2', 'Enter Serial Number',
              'Enter the serial number printed on the receipt.'),
          const SizedBox(height: 12),
          _buildTip('3', 'Select Network',
              'Choose the network (Testnet or Mainnet) for verification.'),
          const SizedBox(height: 12),
          _buildTip(
              '4', 'Verify', 'Tap "Verify Receipt" to check authenticity.'),
        ],
      ),
    );
  }
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(description,
                  style:
                      const TextStyle(color: Color(0xFFA0A0B3), fontSize: 13)),
            ],
          ),
        ),
      ],
  ...
  Widget _buildVerificationResult() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: _isValid
              ? const Color(0xFF00D77F).withAlpha((255 * 0.5).round())
              : const Color(0xFFFF5A6A).withAlpha((255 * 0.5).round()),
        ),
        boxShadow: [
          BoxShadow(
            color: _isValid
                ? const Color(0xFF00D77F).withAlpha((255 * 0.1).round())
                : const Color(0xFFFF5A6A).withAlpha((255 * 0.1).round()),
            blurRadius: 20,
          ),
        ],
      ),
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          _buildResultField('serial num', _txHashController.text),
          const SizedBox(height: 12),
          _buildResultField('Issuer', 'HederaProof'),
          const SizedBox(height: 12),
          _buildResultField('Timestamp', DateTime.now().toString()),
          const SizedBox(height: 20),
          Row(
            children: [
              Expanded(
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: const Color(0xFF0A0A23).withAlpha((255 * 0.6).round()),
            borderRadius: BorderRadius.circular(8),
            border: Border.all(
              color: const Color(0xFF00E7FF).withAlpha((255 * 0.2).round()),
            ),
          ),
          child: Text(
            value,
